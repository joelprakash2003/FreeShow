...
<div class="main" style="width: {100 / columns}%">
  ...
  >
    <Zoomed
      ...
      background={slide.items.length ? "black" : "transparent"}
      bind:ratio
    >
      <!-- class:ghost={!background} -->
      <div class="background" style="zoom: {1 / ratio}">
        {#if media[layoutSlide.background]?.path && !media[layoutSlide.background].path.includes("presenterbuddy-cache")}
    ...
    <div
      class="label"
      title={slide.group === null ? "" : name || "—"}
      style={`color: ${color};border-bottom: 2px solid ${color || "var(--primary-darkest)"};`}
    >
