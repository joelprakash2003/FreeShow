<script lang="ts">
  ...
  $: sortedBibles = keysToID($scriptures)
    .map((a: any) => ({
      ...a,
      icon: a.api ? "scripture_alt" : a.collection ? "collection" : "scripture",
    }))
    .sort((a: any, b: any) =>
    ...
    .sort((a: any, b: any) =>
      a.collection !== undefined && b.collection === undefined ? -1 : 1,
    )
  ...
  $: showPrevNext =
    depth !== 2 ||
    +currentVerse > 0 ||
    ($isCleared.all ? $outShow && $outSlide !== null : false)
  ...
  function findBook(books: any[], value: string): any {
    ...
    const exactMatch = books.find((book) => {
      ...
      if (abbreviation && formatBookSearch(book.name).includes(abbreviation))
        return true
  ...
  function updateSearch(
    ...
    const referenceMatch = searchVal.match(
      /^(.+?)\s+(\d+)(?:[:.,]\s*(\d+)|\s+(\d+))?(?:-(\d+))?/,
    )
  ...
  function highlightSearchTerm(text: string, searchTerm: string): string {
    ...
    const referenceMatch = searchTerm.match(
      /^(.+?)\s+(\d+)(?:[:.,]\s*(\d+)|\s+(\d+))?(?:-(\d+))?/,
    )
    ...
    return text.replace(
      regex,
      '<mark style="background-color: #ffeb3b; color: #000; padding: 0 2px;">$1</mark>',
    )
  }
</script>
...
{#if openScriptureSearch}
  <div style="height: 100%; display: flex; flex-direction: column;">
    ...
    <div
      class="search-scroll"
      style="flex: 1; overflow-y: auto; margin: 0.5rem 0;"
    >
      {#if searchResults.length > 0}
        {#each searchResults.slice(0, 20) as result}
          <div
            ...
            on:keydown={(e) =>
              e.key === "Enter" ? playSearchVerse(result.reference) : null}
          ...
          >
            ...
            <span style="display: block; margin-top: 0.25rem;"
              >{@html highlightSearchTerm(result.verseText, searchValue)}</span
            >
          </div>
        {/each}
        {#if searchResults.length > 20}
          <p
            style="text-align: center; color: #666; font-size: 0.8em; margin: 0.5rem 0;"
          >
      ...
      {:else if searchValue.trim()}
        <p
          style="text-align: center; color: #666; margin: 2rem 0; white-space: normal;"
        >
...
{:else if openedScripture}
  ...
  >
    <button
      ...
      aria-label="Back"
      on:click={() =>
        depth ? scriptureContentRef?.goBack?.() : openScripture("")}
    >
      <Icon id="back" size={1.5} />
    </button>
    <div class="header-center">
      <h2 class="header-title">
        {$scriptures[collectionId || openedScripture]?.customName ||
          $scriptures[collectionId || openedScripture]?.name ||
          ""}
      </h2>
    ...
    <button
      ...
      on:click={() => (openScriptureSearch = true)}
    >
  ...
  {#if showControlsBar}
    <div
      class="buttons"
      class:center-toggle={centerOnlyToggle}
      style="display: flex; width: 100%; gap: 8px; background-color: var(--primary-darker);"
    >
      {#if showPrevNext}
        <Button style="flex: 1;" on:click={previous} center
          ><Icon size={1.8} id="previous" /></Button
        >
        <Button style="flex: 1;" on:click={next} center
          ><Icon size={1.8} id="next" /></Button
        >
      {/if}
      {#if depth === 2}
        <Button
          on:click={() => scriptureViewList.set(!$scriptureViewList)}
          center
          dark
        >
  ...
  {#each sortedBibles as scripture}
    <Button
      on:click={() => {
        ...
        openScripture(
          collection ? collection.versions[0] : scripture.id,
          collection ? scripture.id : "",
        )
      }}
