<script lang="ts">
  ...
  $: if (activeShowId && scrollElem && scrollElem.scrollTop < 10) {
    let activeElement = [...scrollElem.children].find(
      (a) => a.id === activeShowId,
  ...
  function updateScrollbarMetrics() {
    ...
    if (cachedPaddingBottom === 0) {
      const paddingBottomStr =
        getComputedStyle(scrollElem).paddingBottom || "0px"
      cachedPaddingBottom = Number(paddingBottomStr.replace("px", "")) || 0
    }
    const effectiveClientHeight = Math.max(
      0,
      clientHeight - cachedPaddingBottom,
    )
...
{#if $shows.length}
  {#if $shows.length < 10 || loadingStarted}
    <input
      ...
      placeholder="Search..."
      value={searchValue}
      on:input={updateTextValue}
      on:keydown={showSearchKeydown}
      on:click={select}
...
{/each} -->
    <div class="scroll-wrap">
      <div
        class="scroll"
        class:hide-native={enableCustomScrollbar}
      ...
      >
        ...
        {#if enableCustomScrollbar}
          <div
            class="scrollbar"
            style={`top:${scrollElem?.getBoundingClientRect ? scrollElem.getBoundingClientRect().top + window.scrollY : 0}px; bottom:var(--bottom-actions-height, 96px);`}
          >
            <div
        ...
        {#if !tablet}
          <div class="check">
            <p style="font-size: 0.8em;">
              {translate("remote.quick_play", $dictionary)}
            </p>
