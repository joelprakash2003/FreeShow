<script lang="ts">
  ...
  $: {
    if (lyricsScroll && slideNum !== null && $activeTab === "lyrics") {
      let offset =
        lyricsScroll.children[slideNum]?.offsetTop - lyricsScroll.offsetTop - 5
      lyricsScroll.scrollTo(0, offset)
    }
  }

  $: layouts = Object.entries($activeShow?.layouts || {}).map(
    ([id, a]: any) => ({ id, name: a.name }),
  )
  ...
  function playSlide(index: number) {
    ...
    if (
      $outShow &&
      showId === $outShow.id &&
      layoutId === $outShow.settings.activeLayout &&
      index === slideNum
    ) {
      // reveal lines if it exists
      const ref = GetLayout($activeShow, $activeShow?.settings?.activeLayout)
      const revealExists = $activeShow.slides[ref[index]?.id]?.items?.find(
        (item) => item.lineReveal || item.clickReveal,
      )
      if (revealExists) {
...
{#if $activeShow?.layouts}
  ...
  {:else}
    <div
      ...
      class="scroll"
      style="background-color: var(--primary-darker);scroll-behavior: smooth;"
    >
      <Slides
        {dictionary}
        {scrollElem}
        on:click={(e) => playSlide(e.detail)}
        outSlide={slideNum}
      />
    </div>
    ...
    {#if $activeShow.id === $outShow?.id || !$isCleared.all}
      ...
      {#if $activeShow.id === $outShow?.id}
        <div class="buttons" style="display: flex;width: 100%;">
          ...
          <Button
            style="flex: 1;"
            on:click={() => send("API:previous_slide")}
            disabled={slideNum <= 0}
            center><Icon size={1.8} id="previous" /></Button
          >
          <span
            style="flex: 3;align-self: center;text-align: center;opacity: 0.8;font-size: 0.8em;"
            >{slideNum + 1}/{totalSlides}</span
          >
          <Button
            style="flex: 1;"
            on:click={() => send("API:next_slide")}
            disabled={slideNum + 1 >= totalSlides}
            center><Icon size={1.8} id="next" /></Button
          >
    ...
    {:else}
      <div class="buttons">
        {#if layouts.length > 1}
          <Dropdown
            value={layouts.find(
              (a) => a.id == $activeShow.settings?.activeLayout,
            )?.name || "—"}
            options={layouts}
