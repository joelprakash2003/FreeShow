  ...
  {#if editProject}
    <div class="list">
      <div class="header" style="padding: 0;">
        <p style="flex: 1;text-align: center;padding: 0.2em 0.8em;">
          {$activeProject.name}
        </p>
      </div>
        ...
        {#each $activeProject.shows as show, i}
          ...
          <div class="item">
            <p style="padding: 4px 8px;">
              {s?.name ||
                show.name ||
                (show.id ? getFileName(removeExtension(show.id)) : show.type)}
            </p>

            <Button
              style="padding: 4px 8px;"
              on:click={() => removeProjectItem(i)}
              dark
            >
              <Icon id="delete" />
            </Button>
      ...
      <Button on:click={renameProject} style="width: 100%;" center dark>
        ...
        <p style="font-size: 0.8em;">
          {translate("actions.rename", $dictionary)}
        </p>
      </Button>
      ...
      <Button on:click={deleteProject} style="width: 100%;" center dark>
        ...
        <p style="font-size: 0.8em;">
          {translate("actions.delete", $dictionary)}
        </p>
      </Button>
    </div>

    <Button
      on:click={() => (editProject = false)}
      style="width: 100%;border-top: 2px solid var(--primary);"
      center
      dark
    >
      <Icon id="back" right />
      ...
      <div class="scroll">
        {#each $activeProject.shows as show}
          ...
          {#if show.type === "section"}
            <div class="section">
              <p style={show.name ? "" : "opacity: 0.5;"}>
                {show.name || translate("main.unnamed", $dictionary)}
              </p>
            </div>
          {:else if ["image", "video", "overlay", "audio", "pdf"].includes(show.type || "")}
            <Button
              on:click={() => {
                ...
                _set("activeTab", "show")
                if (
                  ["image", "video"].includes(show.type || "") &&
                  !$mediaCache[show.id]
                )
            ...
            >
              ...
              >
                <p
                  style="margin: 3px 5px;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;"
                >
                  {show.name || getFileName(removeExtension(show.id))}
                </p>
              </span>
            </Button>
          {:else if (show.type || "show") !== "show"}
            ...
            <div
              class="item"
              style="display: flex;align-items: center;padding: 0.2em 0.8em;"
            >
              <Icon
                id={show.type || ""}
                box={show.type === "ppt" ? 50 : 24}
                right
              />
              <p
                style="font-size: 0.7em;opacity: 0.5;margin: 3px 5px;text-transform: uppercase;font-size: 0.8em;"
              >
                {show.type}
              </p>
            </div>
