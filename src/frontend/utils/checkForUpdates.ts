...
export function checkForUpdates(currentVersion: string) {
    ...
    fetch("https://api.github.com/repos/ChurchApps/presenterbuddy/releases")
        ...
        .then((data) => {
            ...
            alertMessage.set(
                `<h2>${get(dictionary).about?.new_update || "New update available"}: v${latestVersion}</h2>${get(dictionary).about?.download || "Go to presenterbuddy.app to download"}!<br><br><h3>${get(dictionary).about?.changes || "What's new"
