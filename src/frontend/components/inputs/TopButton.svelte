<script lang="ts">
  ...
  const keys = { show: 1, edit: 2, stage: 3, settings: 5 }

  $: numberKeys = !!$special.numberKeys
  $: title =
    $dictionary.menu?.["_title_" + id] +
    (numberKeys ? "" : ` [${keys[id] || ""}]`)
  ...
  function openActiveShow() {
    ...
    let noEditSlide =
      $activeEdit.slide === null || $activeEdit.slide === undefined
    if (showIsActive && noEditSlide) updateEditItem()
    else if (
      showIsActive &&
      $activeEdit.showId &&
      $activeEdit.showId !== $activeShow?.id
    )
      openEdit()
    ...
    function updateEditItem() {
      ...
      if (
        $activeEdit.id &&
        (!$editHistory.find((a) => $activeEdit.id === a.edit?.id) ||
          $editHistory.find((a) => $activeShow?.id === a.show?.id))
      )
        return
...
<div>
  <!-- width: 140px; -->
  <Button
    style={label ? "padding: 0.3em 1.2em;" : ""}
    {title}
    {disabled}
