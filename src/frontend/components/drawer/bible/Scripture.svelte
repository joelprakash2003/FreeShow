<script lang="ts">
  ...
  $: if (bibles[0]?.verses || $scriptureSettings.longVersesChars)
    updateSplitted()
  $: if (biblePreviewIndex !== undefined) updateSplitted()
  function updateSplitted() {
    if (!$scriptureSettings.splitLongVerses) {
      splittedVerses[displayedBibleId] = Object.entries(
        verses?.[displayedBibleId] || {},
      ).map(([id, text]) => ({ id: id.toString(), text }))
      return
    }

    const chars = Number($scriptureSettings.longVersesChars || 100)
    const newVerses: { id: string; text: string }[] = []
    Object.keys(verses?.[displayedBibleId] || {}).forEach((verseKey) => {
      let verse = verses[displayedBibleId][verseKey]
    ...
    splittedVerses[displayedBibleId] = newVerses
  }
  ...
  function createBiblesList() {
    ...
    if (selectedScriptureData.collection?.versions)
      versions = selectedScriptureData.collection.versions
  ...
  function getBibleId(index: number, bible: any = null) {
    let selectedScriptureData = $scriptures[active || ""]
    let bibleId =
      selectedScriptureData?.collection?.versions?.[index] ||
      selectedScriptureData?.id ||
      bible?.id ||
      active
    return bibleId
  }
  ...
  async function loadAPIBible(bibleId: string, load: string, index = 0) {
    ...
    if (typeof chapterId === "number")
      chapterId = bookId + "." + (chapterId + 1)

    let objectId =
      Object.entries($scriptures).find(([_id, a]) => a.id === bibleId)?.[0] ||
      ""
    if (load === "books" && $scriptures[objectId]?.books2) {
      ...
      try {
        ...
        data = await fetchBible(load, apiId, {
          versesList: versesList[bibleId] || [],
          bookId,
          chapterId,
        })
  ...
  let listenerId = receiveMain(Main.BIBLE, (data) => {
    ...
    if (!bibles[currentIndex])
      return console.error("could not find bible at index")
  ...
  $: if (Object.keys(verses?.[displayedBibleId] || {})?.length) getVerses()
    ...
    bibles.forEach((bible, i) => {
      ...
      if (bibles[i].api) loadAPIBible(id, "books", i)
        ...
        if (typeof bookId === "string")
          bookId = bookIds.findIndex((a) => a === bookId)
  ...
  function getBook() {
    bibles.forEach((bible, i) => {
      ...
      if (bible.api) {
        ...
        bibles[i].book =
          books[id][bookId].customName || books[id][bookId].name || ""
        chapters[id] = (books[id][bookId] as any).chapters

        chapterId = cachedRef?.chapterId || 0
        if (typeof chapterId === "string")
          chapterId = Number(chapterId.split(".")[1]) - 1
  ...
  function selectFirstVerse(bibleId: string, index: number) {
    ...
    if (
      cachedRef?.activeVerses?.length &&
      verses[bibleId]?.[cachedRef?.activeVerses[0]]
    ) {
      activeVerses = cachedRef.activeVerses
    } else if (loaded) {
      activeVerses = activeVerses.length
        ? activeVerses.filter((a) => verses[bibleId]?.[a])
        : ["1"]
    }
  ...
  function selectVerse(e: any, id: string) {
    autoComplete = false
    const rightClick: boolean =
      e.button === 2 ||
      e.buttons === 2 ||
      ($os.platform === "darwin" && e.ctrlKey)
    ...
    if (e.ctrlKey || e.metaKey) {
      ...
      let found = false
      let sorted = activeVerses.sort(
        (a, b) =>
          currentVerses.findIndex((v) => v.id === a) -
          currentVerses.findIndex((v) => v.id === b),
      )[0]
      let first = id
      let last = sorted
      if (
        currentVerses.findIndex((v) => v.id === id) >
        currentVerses.findIndex((v) => v.id === sorted)
      ) {
        first = last
        last = id
      }
  ...
  function updateSearch() {
    ...
    let splitChar = searchEnd.includes(":")
      ? ":"
      : searchEnd.includes(",")
        ? ","
        : searchEnd.includes(".")
    ...
    if (splittedEnd.length === 1 && splittedEnd[0].endsWith(" "))
      updateSearchValue(searchValue.trim() + ":")
    if (splittedEnd.length === 1 && searchValue.endsWith(" "))
      updateSearchValue(searchValue.trim())
    ...
    if (bibles[0].activeVerses !== searchValues.verses) {
      ...
      if (trimmed.length && !trimmed.endsWith("-") && !trimmed.endsWith("+")) {
        ...
        if (splittedEnd[1].endsWith(" "))
          updateSearchValue(searchValue.trim() + (minus === plus ? "-" : "+"))
      } else {
        updateSearchValue(searchValue.trim())
      }
    }
  ...
  async function searchInBible(e: any) {
    ...
    if (
      previousSearch &&
      searchValue.includes(previousSearch) &&
      (!bible.api || cachedSearches < 5) &&
      contentSearchMatches?.length
    ) {
      matches = contentSearchMatches.filter((a) =>
        formatSearch(a.text).includes(searchValue),
      )
      cachedSearches++
    } else if (bible.api) {
      ...
      matches =
        searchResult?.verses?.map((a) => ({
          book: a.bookId,
          chapter: a.chapterId,
          verse: a.reference.slice(a.reference.indexOf(":") + 1),
            ...
            chapter.verses.forEach((verse) => {
              ...
              if (verseValue.includes(searchValue)) {
                matches.push({
                  ...
                  verse: verse.number,
                  reference: `${book.customName || book.name} ${chapter.number}:${verse.number}`,
                  text: verse.text || verse.value,
                })
              } else {
                let wordInSearch = searchValue.split(" ")
                let matchingWords = wordInSearch.reduce(
                  (count, word) => (count += verseValue.includes(word) ? 1 : 0),
                  0,
                )
  ...
  function findBook() {
    let booksList =
      books[firstBibleId]?.map((b: any, i: number) => ({
        ...b,
        id: b.id || i,
        abbr: b.id,
      })) || []
    ...
    if (searchValue.endsWith(" ") && splittedSearch.length === 2) {
      const book = booksList.find(
        (a) => a.abbr && formatBookSearch(a.abbr) === splittedSearch[0],
      )
      if (book) {
    ...
    splittedSearch.forEach((value) => {
      ...
      booksList.forEach((book: any) => {
        ...
        if (
          ...
          bookName.replaceAll(" ", "").includes(value)
        )
          matchingArray.push(book)
      })
    ...
    if (findMatches.length && !hasNumber(lowerSearch.slice(0, 3))) {
      findMatches = findMatches.filter(
        (a) => !hasNumber(a.customName || a.name),
      )
    }

    let exactMatch = findMatches.find(
      (a: any) =>
        (a.customName || a.name) === searchValues.bookName ||
        formatBookSearch(a.customName || a.name) ===
          formatBookSearch(searchValue),
    )
    if (!exactMatch && findMatches.length !== 1) {
      // autocomplete e.g. "First ..."
      const firstWordMatch = [
        ...new Set(
          findMatches.map((a) => (a.customName || a.name).split(" ")[0]),
        ),
      ]
    ...
    if (
      searchValues.book !== undefined &&
      searchValues.book === (matchingBook.keyName || matchingBook.id)
    )
      return matchingBook.keyName || matchingBook.id

    let fullMatch = formatBookSearch(searchValue).includes(
      formatBookSearch(matchingBook.customName || matchingBook.name) + " ",
    )
    if (fullMatch || !autoComplete)
      return matchingBook.keyName || matchingBook.id
  ...
  function findChapter({ splittedEnd }) {
    ...
    chapters[firstBibleId]?.forEach((c, i) => {
      if (c.keyName?.replace(/\D+/g, "") === chapter)
        formattedChapter = c.keyName
  ...
  function moveSelection(moveLeft: boolean) {
    ...
    if (!activeVerses.length) {
      activeVerses = [
        moveLeft
          ? currentVerses[currentVerses.length - 1].id
          : currentVerses[0].id,
      ]
      bibles[0].activeVerses = activeVerses
      return
    ...
    activeVerses = activeVerses.sort(
      (a, b) =>
        currentVerses.findIndex((v) => v.id === a) -
        currentVerses.findIndex((v) => v.id === b),
    )
    let currentId = (
      moveLeft ? activeVerses[0] : activeVerses.at(-1)
    )?.toString()
    let currentIndex = currentVerses.findIndex((a) => a.id === currentId)
    let changeChapter = moveLeft
      ? currentIndex <= 0
      : currentIndex >= currentVerses.length - 1
    if (changeChapter) {
      // find current chapter
      let notApi = typeof chapterId === "number"
      let chapterIndex = notApi
        ? chapterId
        : chapters[firstBibleId].findIndex((a) => a.keyName === chapterId)
      ...
      newChapter.verses.forEach((verseData) => {
        ...
        for (let i = 0; i < newVerseStrings.length; i++) {
          const key = newVerseStrings.length === 1 ? "" : `_${i + 1}`
          newVerses.push({
            id: verseData.number + key,
            text: newVerseStrings[i],
          })
        }
      })
    ...
    setTimeout(() => {
      ...
      ;[...Array(activeCount)].map((_, i: number) => {
        let newIndex: number = moveLeft
          ? currentIndex - i - 1
          : currentIndex + i + 1
        if (
          (moveLeft ? newIndex > -1 : newIndex <= currentVerses.length) &&
          currentVerses[newIndex]
        )
          newSelection.push(currentVerses[newIndex].id)
      })

      if (newSelection.length) {
        activeVerses = newSelection.sort(
          (a, b) =>
            currentVerses.findIndex((v) => v.id === a) -
            currentVerses.findIndex((v) => v.id === b),
        )
  ...
  $: outputIsScripture =
    $outputs[getActiveOutputs()[0]]?.out?.slide?.id === "temp"
  ...
  $: sortedVerses =
    bibles?.[0]?.activeVerses?.sort((a, b) => Number(a) - Number(b)) || []
  let verseRange = ""
  ...
  $: if (active && chapterId)
    setTimeout(() => scrollToActive(chaptersScrollElem))
  $: if (active && activeVerses?.length < 5)
    setTimeout(() => scrollToActive(versesScrollElem))
  ...
  function getShortName(name: string, i: number) {
    let shortName = isNaN(parseInt(name[0]))
      ? name.slice(0, 3)
      : name.replace(" ", "").slice(0, 4)

    // use four characters if same short name ("Jud"ges="Jud"e)
    if (i === 0) usedNames = []
    if (usedNames.includes(shortName) && shortName.length === 3)
  ...
  $: biblePreviewIndex = active
    ? $scriptures[active]?.biblePreviewIndex || 0
    : 0
  ...
  function updateDisplayedContent() {
    ...
    if (!$scriptureSettings.splitLongVerses) {
      splittedVerses[displayedId] = Object.entries(
        verses[displayedId] || {},
      ).map(([id, text]) => ({ id: id.toString(), text }))
      return
    }
  ...
  $: currentHistory = clone(
    $scriptureHistory.filter((a) => a.id === bibles[0]?.id),
  ).reverse()
  ...
  <div class="main">
      ...
      {#if contentSearchMatches.length}
        <div class="verses">
          {#each contentSearchMatches as match}
            ...
            >
              <span
                style="width: 250px;text-align: start;color: var(--text);"
                class="v">{match.reference}</span
              >{@html formatBibleText(
                match.text.replace(
                  /!\{(.*?)\}!/g,
                  '<span class="wj">$1</span>',
                ),
              )}
            </p>
          {/each}
    ...
    {:else if $scriptureMode === "grid"}
      ...
      <div class="grid">
            ...
            {#key books[firstBibleId]}
              {#each books[firstBibleId] as book, i}
                ...
                {@const color = getColorCode(
                  books[displayedBibleId],
                  book.id ?? i,
                )}
                {@const name =
                  book.abbreviation ||
                  getShortName(book.customName || book.name, i)}
        ...
        <div class="content">
          <div
            class="chapters context #scripture_chapter"
            bind:this={chaptersScrollElem}
            style="text-align: center;"
            class:center={!chapters[displayedBibleId]?.length}
          >
          ...
          <div
            class="verses context #scripture_verse"
            bind:this={versesScrollElem}
            class:center={!Object.keys(splittedVerses[firstBibleId] || {})
              .length}
          >
              ...
              {#each currentVerses as content}
                ...
                <span
                  ...
                  on:mousedown={(e) => {
                    ...
                    if (!activeVerses.includes(content.id))
                      activeVerses = [content.id]
                    updateActiveVerses()
                  }}
                  on:dblclick={(e) =>
                    outputIsScripture && !e.ctrlKey && !e.metaKey
                      ? false
                      : playOrClearScripture(true)}
                  on:click={(e) =>
                    outputIsScripture && !e.ctrlKey && !e.metaKey
                  ...
                  class:active={activeVerses.includes(content.id) ||
                    activeVerses.includes(id)}
                  data-title={$dictionary.tooltip?.scripture}
                >
                  {id}
                  <!-- WIP style position not very good -->
                  {#if subverse}<span
                      style="padding: 0;color: var(--text);opacity: 0.5;font-size: 0.8em;"
                      >{getVersePartLetter(subverse)}</span
                    >{/if}
                </span>
              {/each}
            {:else}
    ...
    {:else}
      ...
      <div
        class="books"
        bind:this={booksScrollElem}
        class:center={!books[firstBibleId]?.length}
      >
        {#if books[firstBibleId]?.length}
          {#key books[firstBibleId]}
            {#each books[firstBibleId] as book, i}
              ...
              <span
                ...
                class={bibles[0].api ||
                !Object.values(defaultBibleBookNames).includes(book.name)
                  ? ""
                  : "context #bible_book_local"}
                class:active={bibles[0].api
                  ? bookId === book.keyName
                  : bookId === i}
                style={color ? `border-inline-start: 2px solid ${color};` : ""}
              >
                {$customScriptureBooks[active || ""]?.[id] ||
                  book.customName ||
                  book.name}
              </span>
            {/each}
      ...
      <div
        class="verses context #scripture_verse"
        bind:this={versesScrollElem}
        class:center={!Object.keys(splittedVerses[displayedBibleId] || {})
          .length}
      >
        {#if Object.keys(splittedVerses[displayedBibleId] || {}).length}
          {#each currentVerses as content}
            {@const splitted = content.id.split("_")}
            {@const id = splitted[0]}
            {@const subverse = Number(splitted[1] || 0)}
...
<div class="tabs" style="display: flex;align-items: center;">
  {#if searchBibleActive}
    <TextInput
      placeholder={$dictionary.scripture?.search}
      value={contentSearch}
      on:input={searchValueChanged}
      on:change={searchInBible}
  ...
  <!-- text-align: center; -->
  <span
    style="flex: 1;padding: 0 10px;display: flex;gap: 5px;align-items: center;{bibles.length >
    1
      ? 'padding-left: 0;'
      : ''}"
  >
    {#if displayedBible?.version}
      ...
      {#if bibles.length > 1}
        <Button
          ...
          title={bibles[(biblePreviewIndex + 1) % bibles.length]?.version || ""}
  ...
  {#if !searchBibleActive}
    <Button
      disabled={activeVerses.includes("1") &&
        (chapterId <= 0 || chapterId.toString() === `${bookId}.1`)}
      title={$dictionary.preview?._previous_slide}
      on:click={() => moveSelection(true)}
    >
      <Icon size={1.3} id="previous" />
    </Button>
    <Button
      disabled={$outLocked}
      title={outputIsScripture
        ? $dictionary.preview?._update
        : $dictionary.menu?._title_display}
      on:click={() => playOrClearScripture(true)}
    >
      <Icon
        size={outputIsScripture ? 1.1 : 1.3}
        id={outputIsScripture ? "refresh" : "play"}
        white={!outputIsScripture}
      />
    </Button>
    <Button
      disabled={Object.keys(verses[displayedBibleId] || {}).length &&
        activeVerses.includes(
          Object.keys(verses[displayedBibleId] || {}).length.toString(),
        ) &&
        (chapterId >= chapters[displayedBibleId].length - 1 ||
          chapterId.toString() ===
            `${bookId}.${chapters[displayedBibleId].length + 1}`)}
    ...
    <Button
      disabled={history}
      on:click={() =>
        scriptureMode.set($scriptureMode === "grid" ? "list" : "grid")}
      title={$dictionary.show?.[$scriptureMode === "grid" ? "grid" : "list"]}
    >
    ...
    <Button
      title={$dictionary.scripture?.search}
      on:click={() => (searchBibleActive = true)}
    >
