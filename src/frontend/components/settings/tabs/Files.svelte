<script lang="ts">
  ...
  async function toggle(e: any, key: string) {
    ...
    if (key === "customUserDataLocation") {
      ...
      if (checked) {
        existingData = (
          await requestMain(Main.DOES_PATH_EXIST, {
            path: "data_config",
            dataPath: $dataPath,
          })
        )?.exists
      ...
      if (!existingData) {
        ...
        save(false, {
          backup: true,
          isAutoBackup: true,
          changeUserData: { reset: !checked, dataPath: $dataPath },
        })
      }
    } else {
      updateSpecial(checked, key)
  ...
  $: validKeys =
    typeof $driveKeys === "object" && Object.keys($driveKeys).length
  ...
  function toggleData(e: any, key, invert = false) {
    let checked: boolean =
      typeof e === "boolean" ? e : e.target.checked || false
  ...
  const infoStyle =
    "color: var(--text);opacity: 0.5;font-weight: normal;font-size: 0.8em;margin-left: 10px;"
  $: autosaveInfo = nextAutosave
    ? `<span style="${infoStyle}">${joinTimeBig(nextAutosave / 1000)}<span>`
    : ""
  $: autoBackupInfo = nextAutobackup
    ? `<span style="${infoStyle}">${joinTimeBig(nextAutobackup < 0 ? 0 : nextAutobackup / 1000)}<span>`
    : ""
  $: autoBackup = $special.autoBackup || "weekly"
</script>

<MaterialDropdown
  label="settings.autosave{autosaveInfo}"
  value={$autosave}
  defaultValue="never"
  options={autosaveList}
  on:change={(e) => autosave.set(e.detail)}
/>
<MaterialDropdown
  label="settings.auto_backup{autoBackupInfo}"
  value={autoBackup}
  defaultValue="weekly"
  options={autobackupList}
  on:change={(e) => updateSpecial(e.detail, "autoBackup")}
/>

<MaterialFolderPicker
  label="settings.data_location"
  value={$dataPath}
  on:change={(e) => dataPath.set(e.detail)}
/>
<!-- shows path should be a "Shows" folder inside of "Data location" -->
{#if !$showsPath?.includes($dataPath) || !$showsPath?.includes("Shows")}
  <MaterialFolderPicker
    label="settings.show_location"
    value={$showsPath || ""}
    on:change={(e) => showsPath.set(e.detail)}
  />
{/if}

<MaterialToggleSwitch
  label="settings.user_data_location"
  disabled={!$dataPath}
  checked={$special.customUserDataLocation || false}
  defaultValue={false}
  on:change={(e) => toggle(e.detail, "customUserDataLocation")}
/>
...
<MaterialMediaPicker
  label="cloud.google_drive_api"
  title="cloud.select_key"
  value={validKeys ? translateText("cloud.update_key") : ""}
  filter={{ name: "Key file", extensions: ["json"] }}
  icon="key"
  on:change={receiveKeysFile}
  allowEmpty
/>
<MaterialToggleSwitch
  label="cloud.disable_upload"
  checked={$driveData.disableUpload}
  defaultValue={false}
  on:change={(e) => toggleData(e.detail, "disableUpload")}
/>

{#if validKeys}
  <MaterialToggleSwitch
    label="cloud.enable"
    checked={!$driveData.disabled}
    defaultValue={true}
    on:change={(e) => toggleData(e.detail, "disabled", true)}
  />
  <MaterialTextInput
    label="cloud.media_id"
    value={$driveData?.mediaId || "default"}
    defaultValue="default"
    on:change={(e) => updateValue(e.detail, "mediaId")}
  />
  <MaterialTextInput
    label="cloud.main_folder{$driveData?.mainFolderId
      ? `<span style="margin-left: 10px;font-size: 0.7em;opacity: 0.5;color: var(--text);">drive.google.com/drive/folders/</span>`
      : ''}"
...
{:else}
  <span class="guide" style="display: block;margin-top: 8px;">
    ...
    <p>
      <T id="cloud.tip_how" />&nbsp;<Link
        url={"https://presenterbuddy.app/docs/drive"}
        ><T id="cloud.tip_guide" /></Link
      >
    </p>
  </span>
{/if}
