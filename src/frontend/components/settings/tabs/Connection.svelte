<script lang="ts">
  ...
  function toggleServer(e: any, id: string) {
    ...
    if (id === "output_stream") {
      if (
        $serverData?.output_stream?.outputId &&
        !$outputs[$serverData.output_stream.outputId]
      ) {
        serverData.update((a) => {
  ...
  $: if (
    $activeTriggerFunction.includes("open_connection_") &&
    ip !== "localhost"
  )
    openConnection()
  ...
  const servers = [
    ...
    // Bitfocus Companion (WebSocket/REST)
    {
      ...
      icon: "companion",
      enabledByDefault: false,
      url: "https://presenterbuddy.app/docs/companion",
    },
    // { id: "rest", name: "REST Listener", icon: "companion", enabledByDefault: false, url: "https://presenterbuddy.app/docs/api" },
  ]
  // Camera
  // Answer / Guess / Poll
  ...
  function pcoConnect() {
    if (!$pcoConnected)
      sendMain(Main.PCO_LOAD_SERVICES, { dataPath: $dataPath })
    else {
  ...
  {@const disabled =
    server.id === "companion"
      ...
      : server.enabledByDefault
        ? $disabledServers[server.id] === true
        : $disabledServers[server.id] !== false}
  {@const connections =
    Object.keys($connections[server.id.toUpperCase()] || {})?.length || 0}
  ...
  <InputRow>
    ...
    >
      ...
      >
        ...
        {#if server.id === "companion"}
          <span style="opacity: 0.5;font-size: 0.7em;margin-left: 5px;"
            >WebSocket/REST/OSC/Companion</span
          >
        {/if}
        {#if connections}
          <span style="opacity: 0.5;font-size: 0.7em;margin-left: 5px;"
      ...
      {#if server.id === "output_stream" && $serverData.output_stream?.sendAudio}
        <span
          style="border: none;display: flex;align-items: center;justify-content: end;"
        >
          <Icon id="volume" />
        </span>
      {/if}
    ...
    {#if server.id === "companion"}
      <MaterialToggleSwitch
        label=""
        checked={$companion?.enabled === true}
        on:change={toggleCompanion}
      />
    {:else}
      <MaterialToggleSwitch
        label=""
        checked={server.enabledByDefault
          ? $disabledServers[server.id] !== true
          : $disabledServers[server.id] === false}
        on:change={(e) => toggleServer(e, server.id)}
      />
    {/if}
  </InputRow>
{/each}
...
<InputRow>
  <MaterialButton
    on:click={pcoConnect}
    style="flex: 1;border-bottom: 2px solid var(--{$pcoConnected
      ? 'connected'
      : 'disconnected'}) !important;"
    icon={$pcoConnected ? "logout" : "login"}
  >
    <T
      id="settings.{$pcoConnected ? 'disconnect_from' : 'connect_to'}"
...
<InputRow>
  <MaterialButton
    on:click={chumsConnect}
    style="flex: 1;border-bottom: 2px solid var(--{$chumsConnected
      ? 'connected'
      : 'disconnected'}) !important;"
    icon={$chumsConnected ? "logout" : "login"}
  >
    <T
      id="settings.{$chumsConnected ? 'disconnect_from' : 'connect_to'}"
      replace={["Chums"]}
    />
  </MaterialButton>
  {#if $chumsConnected}
    ...
    <MaterialButton
      title="chums.sync_categories_description"
      icon="options"
      on:click={() => activePopup.set("chums_sync_categories")}
    >
