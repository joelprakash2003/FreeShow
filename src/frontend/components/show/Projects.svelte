<script lang="ts">
  ...
  $: p = Object.entries($projects)
    ...
    .map(([id, project]) => ({
      ...project,
      parent: $folders[project.parent] ? project.parent : "/",
      id,
      shows: [] as any,
    }))
  $: templates = sortByName(keysToID($projectTemplates)).filter(
    (a) => !a.deleted,
  )
  ...
  $: {
    ...
    if (sortType === "created") {
      sortedFolders = sortedFolders.sort(
    ...
    tree = tree.map((a) => ({
      ...a,
      parent:
        !$folders[a.parent] || $folders[a.parent].deleted ? "/" : a.parent,
    }))
  ...
  $: offset = autoscroll(
    scrollElem,
    Math.max(0, ($activeShow?.index || 0) - itemsBefore),
  )
  ...
  $: if (
    ...
    $projects[$activeProject]?.shows?.[$activeShow.index]?.id !==
      $activeShow?.id
  )
    findShowInProject()
  ...
  function findShowInProject() {
    ...
    let i = $projects[$activeProject!].shows.findIndex(
      (p) => p.id === $activeShow?.id,
    )
  ...
  // pre v0.6.1
  $: if ($activeProject && $projects[$activeProject]?.notes !== undefined)
    moveNotes()
  function moveNotes() {
    projects.update((a) => {
      let notes: string = a[$activeProject!].notes || ""
      if (notes.length)
        a[$activeProject!].shows.splice(0, 0, {
          id: "notes",
          type: "section",
          name: "Notes",
          notes,
  ...
  function addSection() {
    let activeShowIndex =
      $activeShow?.index !== undefined ? $activeShow?.index + 1 : null
    let index: number =
      activeShowIndex ?? $projects[$activeProject || ""]?.shows?.length ?? 0
    history({
      id: "UPDATE",
  ...
  $: activeProjectParent = $activeProject
    ? $projects[$activeProject]?.parent
    : ""
  $: projectReadOnly =
    readOnly ||
    profile[activeProjectParent] === "read" ||
    tree.find((a) => a.id === activeProjectParent)?.readOnly

  function createProject(folder: boolean = false) {
    let parent =
      interactedFolder ||
      ($folders[$projects[$activeProject || ""]?.parent]
        ? $projects[$activeProject || ""]?.parent || "/"
        : "/")
    if (
      profile[parent] === "none" ||
      tree.find((a) => a.id === parent)?.readOnly
    )
      parent = "/"
    history({
      id: "UPDATE",
      newData: { replace: { parent } },
      location: { page: "show", id: `project${folder ? "_folder" : ""}` },
    })
  }
  ...
  $: if (listScrollElem) {
    ...
    setTimeout(() => {
      ...
      listOffset = autoscroll(
        listScrollElem,
        Math.max(
          0,
          [
            ...(listScrollElem.querySelector(".ParentBlock")?.children || []),
          ].findIndex((a) => a?.querySelector(".active")) - itemsBefore,
        ),
      )
    }, time)
  }
  ...
  function lastUsed() {
    ...
    recentlyUsedList = keysToID($projects).filter(
      (a) => a.used && Date.now() - a.used < FIVE_DAYS,
    )
    // last used first
  ...
  // most recently interacted with folder (to put New set inside)
...
<div class="main">
    ...
    {#if projectActive || recentlyUsedList.length}
      {#if !$focusMode}
        <Button
          ...
          center
          dark
          title={$dictionary.remote?.projects}
        >
          <Icon id="back" size={1.2} />
        </Button>
        <div
          style="flex: 7;max-width: calc(100% - 43px);"
          class="header context #projectTab _close"
          title={$dictionary.remote?.project +
            ": " +
  ...
  {:else if !projectActive}
    <div
      id="projectsArea"
      class:float={!templates.length}
      class="list projects {readOnly ? '' : 'context #projects'}"
    >
      <Autoscroll
        offset={listOffset}
        bind:scrollElem={listScrollElem}
        timeout={150}
        smoothTimeout={0}
      >
    ...
    <div style="display: flex;">
      <span class="buttons left" style="position: relative;flex: 1;">
        <Button
          style="height: 60px; width: 100%; border: 0.25px solid white;"
          disabled={readOnly}
          title="new.folder"
      ...
      <span class="buttons right" style="position: relative;flex: 5;">
        <div style="display: flex; flex-direction: column; height: 60px;">
          <Button
            style="height: 30px; width: 100%; border: 0.25px solid white;"
            disabled={readOnly}
            title="tooltip.join_set"
            on:click={() => activePopup.set("join_set")}
            center
            dark
          >
            <Icon id="connection" white />
            {#if !$labelsDisabled}<T id="join.set" />{/if}
          </Button>
          <Button
            style="height: 30px; width: 100%; border: 0.25px solid white;"
            disabled={readOnly}
  ...
  {:else}
    <div
      id="projectArea"
      class="list {projectReadOnly ? '' : 'context #project'}"
    >
        ...
        <DropArea id="project" selectChildren let:fileOver file>
          {#if $projects[$activeProject || ""]?.shows?.length}
            {#each $projects[$activeProject || ""]?.shows as show, index}
              {@const triggerAction =
                show.data?.settings?.triggerAction ||
                $special.sectionTriggerAction}
              ...
              <SelectElem
                ...
                data={{
                  ...show,
                  name: show.name || removeExtension(getFileName(show.id)),
                  index,
                }}
                {fileOver}
                borders="edges"
              ...
              >
                {#if show.type === "section"}
                  <Button
                    active={$focusMode
                      ? $activeFocus.id === show.id
                      : $activeShow?.id === show.id}
                    class="section {projectReadOnly
                      ? ''
                      : `context #project_section__project ${show.color ? 'color-border' : ''}`}"
                    style="font-weight: bold;{$fullColors
                      ? `background-color: ${show.color};color: ${getContrast(show.color || '')};`
                      : `--border-color: ${show.color};color: ${show.color};`}"
                    on:click={() => {
                      if ($focusMode)
                        activeFocus.set({ id: show.id, index, type: show.type })
                  ...
                  >
                    ...
                    {#if triggerAction && $actions[triggerAction]}
                      <span
                        style="display: flex;position: absolute;inset-inline-end: 5px;"
                        data-title={$actions[triggerAction].name}
                      >
                        <Icon
                          id={getActionIcon(triggerAction)}
                          size={0.8}
                          white
                        />
                      </span>
                ...
                {:else}
                  <ShowButton
                    ...
                    class={projectReadOnly
                      ? ""
                      : `context #${pcoLink ? "pco_item__" : ""}project_${getContextMenuId(show.type)}__project`}
                    icon
                  />
                {/if}
              </SelectElem>
