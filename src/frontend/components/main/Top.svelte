<script type="ts">
  ...
  $: isLocked =
    show?.locked ||
    showProfile.global === "read" ||
    showProfile[show?.category || ""] === "read"

  // && !$editHistory.length
  $: editDisabled =
    $activeEdit.id && ($activeEdit.type || "show") !== "show"
      ? false
      : $activeShow && ($activeShow?.type || "show") === "show"
        ? isLocked
        : $activeShow?.type === "pdf" || !$activeShow?.id
  $: physicalOutputWindows = Object.values($outputs).filter(
    (a) => a.enabled && !a.invisible,
  )
...
<div class="top" class:drag={!isWindows}>
  ...
  <span style="width: var(--navigation-width);">
    ...
    <!-- logo -->
    <h1
      style="align-self: center;width: 100%;padding: 0px 10px;text-align: center;font-size: 1.8em;"
    >
      PresenterBuddy
    </h1>
    <!-- <div class="logo">
            <img style="height: 35px;" src="./assets/Grey_Logo.png" alt="PresenterBuddy-logo" draggable={false} />
            <h1 style="color: var(--text);font-size: 1.7em;">PresenterBuddy</h1>
        </div> -->
  </span>
  <span>
    <TopButton id="show" />
  ...
  <span style="width: var(--navigation-width);justify-content: flex-end;">
    ...
    <!-- <TopButton
      ...
      red={$drawTool === "fill" ||
        $drawTool === "zoom" ||
        !!($drawTool === "paint" && $paintCache?.length)}
      hideLabel
    /> -->
    {#if !settingsDisabled}
    ...
    <Button
      ...
      title={($outputDisplay
        ? confirm
          ? $dictionary.menu?.again_confirm
          : $dictionary.menu?._title_display_stop
        : $dictionary.menu?._title_display) + " [Ctrl+O]"}
