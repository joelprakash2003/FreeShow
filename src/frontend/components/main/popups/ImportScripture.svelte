<script lang="ts">
  ...
  $: {
    if (bibles?.length) {
      ...
      sortedBibles = sortByName(bibles).sort(
        (a, b) =>
          (b.attributionRequired || b.attributionString) -
          (a.attributionRequired || a.attributionString),
      )
      let newSorted: any[] = []
  ...
  function search(e: any) {
    ...
    searchedBibles = sortedBibles.filter((a) =>
      value.split(" ").find((value) => a.name.toLowerCase().includes(value)),
    )
    searchedRecommendedBibles = recommended.filter((a) =>
      value.split(" ").find((value) => a.name.toLowerCase().includes(value)),
    )
  }
  ...
  function goBack() {
    if (
      importType === "api" &&
      searchActive &&
      (document.getElementById("scriptureApiSearchInput") as any)?.value
    ) {
      searchActive = false
      searchedBibles = sortedBibles
      searchedRecommendedBibles = recommended
...
{#if importType === "api"}
  ...
  {:else}
    <div style="display: flex;align-items: center;gap: 10px;">
      {#if searchActive}
        <TextInput
          id="scriptureApiSearchInput"
          style="width: 50%;border-bottom: 2px solid var(--secondary);"
      ...
      {:else}
        <Button
          class="search"
          style="border-bottom: 2px solid var(--secondary);"
          on:click={() => (searchActive = true)}
          bold={false}
        >
          <Icon id="search" size={1.4} white right={!$labelsDisabled} />
          {#if !$labelsDisabled}<p style="opacity: 0.8;font-size: 1.1em;">
              <T id="main.search" />
            </p>{/if}
        </Button>
      {/if}
      ...
      {#if searchedRecommendedBibles.length}
        {#each searchedRecommendedBibles as bible}
          <Button
            ...
            active={!!Object.values($scriptures).find(
              (a) => a.id === bible.sourceKey,
            )}
          >
            ...
            {#if bible.description && bible.description.toLowerCase() !== "common" && !(bible.nameLocal || bible.name).includes(bible.description)}
              <span class="description" data-title={bible.description}
                >({bible.description})</span
              >
            {/if}
          </Button>
        {/each}
      ...
      {#if sortedBibles.length}
        {#if searchedBibles.length}
          {#each searchedBibles as bible}
            <Button
              bold={false}
              on:click={() => toggleScripture(bible)}
              active={!!Object.values($scriptures).find(
                (a) => a.id === bible.sourceKey,
              )}
            >
...
{:else if importType === "local"}
  ...
  <p style="opacity: 0.9;">
    Find some available <Link
      url="https://presenterbuddy.app/resources#scriptures">Bible versions</Link
    >.
  </p>
    ...
    <Button
      on:click={() =>
        sendMain(Main.IMPORT, {
          channel: "BIBLE",
          format: { name: "Bible", extensions: ["xml", "xmm", "json", "fsb"] },
        })}
      style="width: 100%;"
      center
      dark
...
{:else}
  <MaterialMultiChoice
    options={importTypes}
    on:click={(e) => (importType = e.detail)}
  />
{/if}
