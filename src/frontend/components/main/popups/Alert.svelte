<script lang="ts">
  ...
  // UPDATER
  $: if (msg.includes("presenterbuddy.app")) {
    msg = msg.replace(
      "presenterbuddy.app",
      '<a href="#void" class="website">presenterbuddy.app</a>',
    )
  }
  $: if (msg.includes("link#")) {
    msg = msg.replace(
      "link#bible-converter",
      '<a id="bible-converter">Bible Converter</a>',
    )
  }

  function click(e: any) {
    if (e.target.closest(".website")) {
      sendMain(Main.URL, "https://presenterbuddy.app/?download")
...
<p on:click={click}>
  {#key msg}
    {#if msg.includes("captions#")}
      ...
      <br />
      <br />
      <Link url={msg.slice(msg.indexOf("#") + 1)}
        >{msg.slice(msg.indexOf("#") + 1)}</Link
      >
