<script lang="ts">
  ...
  function getList(outputs) {
    let list = keysToID(outputs).filter(
      (a) => !a.isKeyOutput && a.enabled === true,
    )
...
{#if options}
  ...
  {:else if id === "output_stream"}
    ...
    <CombinedInput>
      ...
      <Dropdown
        value={outputsList.find(
          (a) => a.id === $serverData?.output_stream?.outputId,
        )?.name || "—"}
        options={outputsList}
        on:click={(e) => updateData(e.detail.id, "outputId")}
      />
    </CombinedInput>
    <!-- {/if} -->
...
{:else}
  <div on:mousedown={mousedown}>
    {#if id === "companion"}
      <Link url="https://presenterbuddy.app/api">
        API Docs
        <Icon id="launch" white />
      </Link>
      <Link url="https://presenterbuddy.app/docs/companion">
    ...
    {#if id === "remote" && $remotePassword}
      <p style="padding-top: 10px;font-size: 0.9em;">
        <T id="remote.password" />: <b>{$remotePassword}</b>
      </p>
    {/if}
  </div>

  <Button
    style="position: absolute;inset-inline-start: 10px;bottom: 10px;padding: 12px;"
    title={$dictionary.edit?.options}
